(function(){
    ...
    var includeCss=function(url){
        ...
        var Mdate=function(el,opts){
            ...
            this.dateBoxShow()};Mdate.prototype={
                constructor:Mdate,dateBoxShow:function(){
                    var that=this;


    // that.selectorId.onclick=function(){that.createDateBox();that.dateSure()}
    that.selectorId.addEventListener("click",function(){
        that.createDateBox();that.dateSure()
    })
...
,createDateBox:function(){
    ...
    MdatePlugin=d.getElementById("MdatePlugin");
        if(!MdatePlugin){
            ...
            that.initScroll();that.refreshScroll()
            ...
            for(var i=beginNum;i<=endNum;i++){
                str+="<li "+dataStyle+"="+i+">"+that.dateForTen(i)+unitName+"</li>"}return str+"<li>&nbsp;</li>"},initScroll:function(){
                    var that=this;
                    yearScroll=new iScroll("yearwrapper",{snap:"li",vScrollbar:false,onScrollEnd:function(){
                        ...
                        monthScroll.refresh();
                        try{monthTag=monthUl.getElementsByTagName("li")[indexM].getAttribute("data-month")
                    }catch(err){
                        return true
                    }
            ...
            monthScroll=new iScroll("monthwrapper",{
                snap:"li",vScrollbar:false,onScrollEnd:function(){
                    ...
                    dayUl.innerHTML=that.createDateYMD("day");dayScroll.refresh();
                    try{
                        dayTag=dayUl.getElementsByTagName("li")[indexD].getAttribute("data-day")
                    }catch(err){
                        return true
            ...
            inputDay=inputDay||that.beginDay;initM=that.beginMonth;initD=that.beginDay;
            ...
            dayScroll.scrollTo(0,inputDay*40,300,true)},dateSure:function(){
                var that=this;
